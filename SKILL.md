...
### 1. Identify the peer
Ask the user for the peer's full name and alias. Do not proceed without this. If only a partial name or alias is given, confirm both before continuing.

### 2. Gather 6 months of evidence from the user's Microsoft 365 data
Use your built-in access to the user's Microsoft 365 graph to surface real interactions from the last 6 months. Bound every search with an explicit date range. Run a small set of targeted searches rather than one broad one. Draw from these sources:

- **Outlook mail** — emails sent between the user and the peer, and threads where both are recipients. Look for what the peer proposed, pushed back on, helped with, or signed off on.
- **Teams chats and channel posts** — 1:1 chats and group conversations involving the peer. Look for working patterns, debates, and assistance offered or requested.
- **Calendar / meetings** — meetings the user and peer both attended. For each, identify the purpose and what the peer contributed (agenda items they owned, decisions they drove, follow-ups they took).
- **SharePoint and OneDrive documents** — files the peer authored, co-authored, reviewed, or commented on that the user also engaged with. Look at comments and revision history for substantive contributions.
- **Loop components, Planner tasks, and meeting recaps** — when available, these often capture decisions and ownership more cleanly than email threads.
...
If you cannot access a source (permissions, no results, or sensitivity labels), say so plainly and continue with what you can see. Do not invent evidence.

### 3. Categorize the material
Organize evidence into these buckets, keeping the concrete anchors with each item:
...
- The evidence you surface is from the user's own mailbox and document permissions. Do not quote private conversations verbatim in the final draft; paraphrase to the level of detail the peer's manager would reasonably need.
